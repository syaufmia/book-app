...
Eine Büchersuche über die API gibt *immer* den Status Code `200` aus, auch wenn die Suche grundsätzlich keine Ergebnisse vorweist.
Leere Ergebnisse werden durch eine leere Liste in der JSON Response gekennzeichnet.
...
Im Vergleich zur Suche nach Büchern wird bei der Autorensuche eine JSON-Liste nach Autorobjekten ausgegeben. +
...
| OK
| 404
| Wird gesendet, wenn gegebene ID nicht existiert
...
| 404
| Wird gesendet, wenn ISBN nicht existiert
