...
Aktuell ist das Starten der Anwendung ohne einen link:#web_container_[Webcontainer] nicht möglich.
...
../Tomcat8/webapps/
----
So könnte ein Beispielverzeichnis eines link:#web_container_[Webcontainers] aussehen:

image:img/Screenshot 2021-02-16 003239.png[]

Nach dem Verschieben der Datei in den genannten Pfad kann die der link:#web_container_[Webcontainer] gestartet werden. +
...
>startup
----
Mit einem beliebigen Browser kann man den link:#local_server_[lokalen Server] nun durch
`\http://localhost:8080/` erreichen.
Durch Anhängen des Dateinamens an die o.g. URI kann die Anwendung aufgerufen werden.

====
Wenn beispielseise der Dateiname der Applikation `book-app.war` lautet,
dann ist die diese nun über den link über `\http://localhost:8080/book-app/` erreichbar.
====
